...
box "Client Side" #LightBlue
    ...
    participant "Bookie Selector Sheet" as Selector
    participant Betslip
end box
...
alt Config API Success
    ...
    alt ALL 國家 Provider (如 Fcom)
        ...
        note right : Country 預設為 ALL，無需選擇
    else 單一國家 Provider (如 Bet9ja)
        User -> Selector : 選擇 Provider
        note right : Country 自動選定，無需選擇
...
note right : Request: {provider, country, bookingCode}

alt Convert API Success
    BE --> Widget : {bizCode: 10000, data: {shareCode, successCnt, failCnt}}
    note right : 記錄 failCnt 用於後續 Toast 顯示
...
* 📍 **PRD 來源**：Fcom_PRD_Booking_Code_Converter_01_06_zh-TW.md
* 📍 **API 文件**：Code_Converter_API_Doc.md
